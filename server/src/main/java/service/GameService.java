...
public class GameService {
    ...
    public ListResult listGames(String authToken) throws Exception {
        if (authToken == null || authToken.isEmpty()) {
            throw new BadRequestException("auth token must be provided");
        }
    ...
    public void joinGame(String authToken, JoinRequest request) throws Exception {
        if (request.playerColor() == null || request.gameID() <= 0){
            throw new BadRequestException("player color or gameID cannot be null");
        }
        ...
        if (request.playerColor().equals("WHITE")){
            ...
            throw new BadRequestException("player color must be WHITE or BLACK");
        }
